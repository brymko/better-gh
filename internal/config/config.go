...
type Config struct {
	...
	// the public address rather than the backend. Optional for DIRECT serving; REQUIRED behind a
	// TLS-terminating front — otherwise the verification URL points at the loopback backend and
	// `gh auth login` cannot complete (see docs/deployment.md).
