...
- The **`/login/*` and `/ui` endpoints are unauthenticated** (they are the token-*acquisition* surface, mounted on the GHE listener). They do **not** require a proxy token; minting is gated only by the **GitHub owner sign-in** (TOFU — a non-owner cannot mint). So exposing the GHE listener does not let a stranger mint a token, but it does expose an unauthenticated surface (device-flow starts — rate-limited and capped — and the classic **device-code phishing** risk: only complete a sign-in *you* initiated). **Keep the GHE listener network-access-controlled** (Tailscale ACL strongly preferred over a public Caddy domain); see [docs/deployment.md](docs/deployment.md).
...
- **REST enumeration/search responses are redacted too.** The REST endpoints that return repository-bearing entries from many repos — `/user/repos`, `/user/issues`, `/user/starred`, `/user/subscriptions`, `/orgs/{org}/repos`, `/orgs/{org}/issues`, `/users/{u}/{repos,starred,subscriptions}`, `/repos/{owner}/{repo}/forks`, `/issues`, `/notifications`, `/{events}` (incl. `/orgs/{org}/events`, `/users/{u}/events`), the org-wide alert feeds `/orgs/{org}/{secret-scanning,dependabot,code-scanning}/alerts`, `/orgs/{org}/teams/{team}/repos`, and `/search/{repositories,code,issues,commits}` — have denied-repo entries dropped from the response (keyed on `full_name` / `repository.full_name` / `repository_url` / the events `repo.name` / the `star+json` `repo.full_name`). Without this, the `user`/`search`/`notifications` categories — or a `[[org]]`-read token with a per-repo `none` carve-out — could enumerate denied repos' metadata, read their code via `/search/code`, read issue/PR titles via `/notifications`, or read a carved-out repo's **secret-scanning alerts (including the cleartext secret)** via the org alert feeds, sidestepping the GraphQL filter. A search response that drops matches has its `total_count` rewritten to the kept count (and `incomplete_results` set) so the count can't be used as a denied-repo existence oracle. The `/{orgs/{org},user}/migrations` feeds nest a `repositories[]` of many repos per entry; denied repos are dropped from within each entry (the migration metadata is kept). (Defense-in-depth: an off-shape body passes through unchanged; a response too large to buffer for filtering is denied, not forwarded truncated.)
...
- **REST read isolation is an allowlist (best-effort), not fail-closed like GraphQL.** Single-repo REST paths (`/repos/{owner}/{repo}/…`) are soundly scoped by the classifier (the path fixes the repo, policy checks it). The *multi-repo* REST endpoints are redacted by the list above — but that list is an **allowlist**: an endpoint not on it (or a listed one returning an unexpected JSON shape) is **forwarded unredacted** rather than denied (so an allowed list/search never breaks). The GraphQL filter, by contrast, types the whole request and **denies** anything it can't tag. This asymmetry is architectural — REST is opaque JSON the proxy can't generically prove repo-scoped, whereas GraphQL is typed. With no upstream floor, an unrecognized cross-repo REST endpoint is a genuine residual: keep the list current, prefer that clients reach cross-repo data via GraphQL, and use a fine-grained PAT custodian if you need a hard guarantee.
