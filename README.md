...
For commit, PR, and release hygiene, see [CONTRIBUTING.md](CONTRIBUTING.md).
...
> [!IMPORTANT]
> `serve` defaults to **`mode = "socket"`** (what `init` writes), which avoids starting the **GHE HTTPS listener** and its unauthenticated `/login` + `/ui` sign-in surface. The **admin API/UI on `admin_bind`** (a full-access token-minting surface) starts in **every** mode, including `mode = "socket"` — it is not gated by mode. The defaults are still safe only because `bind`/`admin_bind` are **loopback**. Set `mode = "ghe"` for remote-only use, or `mode = "both"` only when you explicitly want the socket and GHE listeners together. **Firewall `admin_bind` regardless of mode**, and put any non-loopback listener behind a network allowlist (see [docs/deployment.md](docs/deployment.md)).
...
`gh` sends its own GitHub token, but the proxy ignores it and uses the **custodian** for upstream requests — the token captured by the first GitHub sign-in (`owner.json`) if the deployment has been claimed, otherwise the pre-seeded fallback (`BGH_GITHUB_TOKEN` / `github_token` / `bgh-proxy login`). In `mode = "both"`, a `gh auth login` / `/ui` sign-in on the GHE listener claims the deployment, and that captured token then backs **socket** traffic too. In the default `mode = "socket"`, there is no GHE sign-in surface; the socket policy alone controls what gets through.
...
The low-level **Git Data API** (`/repos/{owner}/{repo}/git/*`) has no resource key of its own; it is governed by the resource its operation actually touches — `git/refs`, `git/tags` → `branches`; `git/blobs`, `git/trees` → `contents`; `git/commits` → `commits` — so a `branches = "none"` or `contents = "none"` rule covers the equivalent git-plumbing calls (creating/force-pushing a ref, reading raw file bytes) just as it covers the high-level paths. (Over GraphQL, per-resource enforcement on objects reached by navigation comes from the response filter's type→resource map, derived from the embedded schema's `@docsCategory` values — see the [Security model](#security-model).)
...
- Node-ID scoping resolves **every** referenced node ID against GitHub using a query generated from the embedded schema, covering the repo-scoped `Node` types in that schema (both modern and legacy ID forms). A node that doesn't resolve adds no scope (a lone such node is denied as an unscoped write).
