...
> [!IMPORTANT]
> `serve` defaults to **`mode = "both"`** (what `init` writes), which additionally starts the **GHE HTTPS listener** — mounting the *unauthenticated* `/login` + `/ui` sign-in surface — and **always** starts the admin API/UI, even for purely local socket use. This is safe-by-default only because `bind`/`admin_bind` default to **loopback**. For local-only use set `mode = "socket"` to avoid the extra surface; for remote-only use set `mode = "ghe"` and put the listener behind a network allowlist (see [docs/deployment.md](docs/deployment.md)).
