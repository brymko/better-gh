...
- `mode = "allow"` permits anything the classifier cannot map to a deny rule. GraphQL node-ID reads/writes are resolved and checked **when the node's type is repo-scoped**; a node whose type the resolver does not recognize as belonging to a repository (e.g. a `ProjectV2`, which can be repo-owned) adds no scope, so under `allow` a `node(id:)` read of it falls through. (Note: a resolved node whose runtime type the *embedded schema* does not recognize at all — live schema drift — now fails **closed**, so `allow` only lets through node types the snapshot knows but does not class as repo-scoped.) REST is tighter under `allow` than it used to be: because the response filter is always active, a GET whose path the OpenAPI spec does not describe and that is not scoped to one repo is **denied off-spec** (e.g. repo-by-numeric-id `GET /repositories/{id}`), not forwarded. **Use `mode = "deny"` for a safe baseline** anyway; reserve `allow` for low-stakes setups where you accept that anything unmapped-but-on-spec is permitted. With no upstream floor in the default deployment, there is nothing behind `mode = "allow"` to catch what it lets through, so deny-by-default is strongly preferred.
