...
- **The upstream token sees everything, so GraphQL isolation is best-effort.** The proxy denies the *known* cross-repo navigation fields (above, including off `search`/`organization` results), but that is a denylist, not a proof — GraphQL has many ways to reach another repo's data (project items, future schema fields, …) and the proxy can't enumerate them all. **The only hard boundary is the upstream token's own scope:** for real isolation between repos, give the proxy a **fine-grained PAT** limited to the repos it should reach, so GitHub itself blocks the rest, rather than relying on per-repo policy over a broad token. The per-repo policy is best understood as least-privilege + audit on top of an already-scoped token, not a wall around a god-token.
