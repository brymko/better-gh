...
> device-flow URL points at the loopback backend and `gh auth login` breaks — and `mode = "both"`,
> which **also opens the unix socket** and starts the GHE listener's unauthenticated `/login` + `/ui`
> surface; set `mode = "ghe"` for a remote-only listener (or `mode = "socket"` for local-only). The
